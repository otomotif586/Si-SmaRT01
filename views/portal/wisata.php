    <!-- Destinasi Wisata -->
    <section id="wisata" class="py-16 md:py-24 lg:py-32 bg-white/30 backdrop-blur-sm">
        ...
        <div id="wisata-grid" class="container mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
            ...
            <div class="group relative h-[400px] md:h-[500px] overflow-hidden rounded-[3rem] md:rounded-[4rem] reveal shadow-2xl wisata-item paginate-item">
                <img src="<?= htmlspecialchars(smart_asset($wisata[$i]['image']), ENT_QUOTES, 'UTF-8') ?>" class="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-all duration-1000 opacity-60" alt="<?= htmlspecialchars($wisata[$i]['title']) ?>" loading="lazy">
